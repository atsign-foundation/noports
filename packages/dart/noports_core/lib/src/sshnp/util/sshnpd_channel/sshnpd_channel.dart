...
abstract class SshnpdChannel with AsyncInitialization, AtClientBindings {
  ...
  Future<List<(DaemonFeature feature, bool supported, String reason)>> featureCheck(List<DaemonFeature> featuresToCheck,
    ...
    try {
      await atClient.put(AtKey.fromString(locallyCachedPingResponseKey), jsonEncode(pingResponse));
    } catch (e) {
    ...
    twinKeys = (pingResponse!['supportedFeatures']?[DaemonFeature.twinKeys.name] == true);
  ...
  Future<SshnpDeviceList> listDevices({
    ...
    List<SshnpDeviceList> deviceLists =
        await Future.wait(atSigns.map((a) => _listDevices(a, waitDuration: waitDuration)).toList());
