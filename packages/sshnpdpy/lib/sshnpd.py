...
def handle_decryption(
    queue: Queue, client: AtClient, ssh_path, args: argparse.ArgumentParser
):
    private_key = ""
    sshPublicKey = ""
    ...
    while True:
        try:
            ...
            if event_type == AtEventType.DECRYPTED_UPDATE_NOTIFICATION:
                ...
                if event_type == AtEventType.UPDATE_NOTIFICATION:
                    client.secondary_connection.execute_command(
...
def forward_socket_handler(chan, dest):
    sock = socket()
    try:
        sock.connect(dest)
    except Exception as e:
...
def main():
    ...
    # Put the username key
    device_namespace = f"{args.device}.sshnp"
    username_key = SharedKey(
        "username", AtSign(args.device_atsign), AtSign(args.manager_atsign)
    )
    username_key.namespace = device_namespace

    client.put(username_key, args.username)
