...
int run_srv(srv_params_t *params) {
  chunked_transformer_t *encrypter = NULL;
  chunked_transformer_t *decrypter = NULL;
  ...
  if (params->session_aes_key_string != NULL &&
    ...
    // Allocate the encrypter and decrypter
    encrypter = malloc(sizeof(chunked_transformer_t));
    decrypter = malloc(sizeof(chunked_transformer_t));
    ...
    // Load the key into the aes contexts
    mbedtls_aes_init(&encrypter->aes_ctr.ctx);
    ...
    mbedtls_aes_init(&decrypter->aes_ctr.ctx);
