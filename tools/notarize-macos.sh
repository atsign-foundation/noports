...
codesign \
	...
	-s "$SIGNING_IDENTITY" \
	-v \
	"$WORKING_DIR"/sshnp/{ssh*,srv,srvd,at_activate,debug/srvd}

echo Verifying signatures:
codesign -vvv --deep --strict "$WORKING_DIR"/sshnp/{ssh*,srv,srvd,at_activate,debug/srvd}
...
if [ -n "$KEYCHAIN_PROFILE" ]; then
	xcrun notarytool submit "$WORKING_DIR/$OUTPUT_FILE".zip --keychain-profile "$KEYCHAIN_PROFILE" \
		--wait
else
